...
class MDCDConfig:
    ...
    def __init__(self, segment_size=8192): ### add segment size ###
...
class SBD(torch.nn.Module):
    ...
    def __init__(self, use_spectral_norm=False ,segment_size=8192): ### add segment size ###
        super(SBD, self).__init__()
        self.config = MDCDConfig(segment_size)  ### add segment size ###
...
class AvocodoDiscriminator(nn.Module):
    ...
    def __init__(self, use_spectral_norm=False, segment_size=8192): ### add segment size ###
        ...
        self.sbd = SBD(use_spectral_norm, segment_size) ### add segment size ###
...
class SynthesizerTrn(nn.Module):
    ...
    def __init__(
        ...
        ### add ###
        self.w_step = w_step
        self.is_sr_double = int(sr/22050)
        ###########
    ...
    def forward(self,
        ...
        #for yin dec loss
        yin_gt_crop, yin_gt_shifted_crop, yin_dec_crop, z_yin_crop_shifted, scope_shift = self.yin_dec(     ### add description  # yin_gt_crop = [B, Scope, Frame]
        ...
        ### add ### 
        # for Q option
        # z_dec = torch.cat([z_spec_q, z_yin_crop], dim=1) 
        # z_dec_shifted = torch.cat([z_spec_q.detach(), z_yin_crop_shifted], dim=1)
        # z_dec_ = torch.cat([z_dec, z_dec_shifted], dim=0)
        ########### 
        ...
        o_pad = F.pad(o_[-1], (self.filter_channels*self.is_sr_double , self.filter_channels*self.is_sr_double  + (-o_[-1].shape[-1]) % self.w_step + self.w_step *
                               (o_[-1].shape[-1] % self.w_step == 0)),
                      mode='constant').squeeze(1)
        ##############
