...
from data_utils import (TextAudioSpeakerLoader, TextAudioSpeakerCollate,
                        DistributedBucketSampler, create_spec, dataset_check)
from models import (
    SynthesizerTrn,
...
def main(args):
    ...
    os.environ['MASTER_PORT'] = '12345'     ### modify ###

    hps = utils.get_hparams(args)
    # create spectrogram files ### modify ### 
    #create_spec(hps.data.training_files, hps.data)
    #create_spec(hps.data.validation_files, hps.data)
    dataset_check(hps.data.validation_files, hps.data)
    dataset_check(hps.data.training_files, hps.data)
    
    run(rank=0, n_gpus=1, hps=hps, args=args)               ### modify ### 
...
def run(rank, n_gpus, hps, args):
    ...
    net_d = AvocodoDiscriminator(hps.model.use_spectral_norm, hps.train.segment_size).cuda(rank)
    if rank == 0:
    ...
    net_g = DDP(net_g, device_ids=[rank], find_unused_parameters=False) ### modify ###
    net_d = DDP(net_d, device_ids=[rank], find_unused_parameters=False) ### modify ###
    ...
    for batch_idx, (x, x_lengths, spec, spec_lengths, ying, ying_lengths, y,
        ...
        with autocast(enabled=hps.train.fp16_run):
            ...
            with autocast(enabled=False):
                ...
                ### add ### for frame check
                # _, _, gt_length = yin_gt_crop.shape
                # _, _, hat_length = yin_hat_crop.shape
                # if hat_length < gt_length:
                #     yin_gt_crop = yin_gt_crop[:,:,:hat_length]
                #     print(f"yin_gt : {gt_length} | yin_hat : {hat_length}")
                # elif hat_length > gt_length:
                #     yin_hat_crop = yin_hat_crop[:,:,:gt_length]
                #     print(f"yin_gt : {gt_length} | yin_hat : {hat_length}")
                ###########
...
if __name__ == "__main__":
    ...
    ### add ###
    # model_name = input("Enter your experimental model name...")
    ###########
    ...
    parser.add_argument('-c',
                        ...
                        default="./configs/config_ja_44100.yaml",       ### add ###
