...
class TextAudioSpeakerLoader(torch.utils.data.Dataset):
    ...
    def __init__(self, audiopaths_sid_text, hparams, pt_run=False):
        ...
        self.pitch = Pitch(sr=hparams.sampling_rate,
                           ### modify ##############
                           #W=hparams.tau_max,
                           W=hparams.ying_window,           
                           w_step=hparams.ying_hop,
                           ######################### 
...
def create_spec(audiopaths_sid_text, hparams):
    audiopaths_sid_text_list = load_filepaths_and_text(audiopaths_sid_text)
    for audiopath, _, _ in tqdm(audiopaths_sid_text_list):          ### add tqdm### 

        ### modify ###
        audio_path = copy.deepcopy(audiopath)
        audiopath = os.path.join(hparams.data_path, audiopath)
        try:
            audio, sampling_rate = load_wav_to_torch(audiopath)
        except:
            with open(audiopaths_sid_text, mode="r", encoding="utf-8") as f:
                lines = f.readlines()
            for idx, line in enumerate(lines):
                check_path, sentence, spk_id = line.split("|")
                if check_path == audio_path:
                    remove_path = lines.pop(idx)
            with open(audiopaths_sid_text, mode="w", encoding="utf-8") as f:
                f.writelines(lines)
            print(f"File is not found!!! path={remove_path}   ==> Removed.")
            continue
        ##############
...
### add ###
def dataset_check(audiopaths_sid_text, hparams):
    audiopaths_sid_text_list = load_filepaths_and_text(audiopaths_sid_text)
    for audiopath, _, _ in tqdm(audiopaths_sid_text_list):          ### add tqdm### 
...
### add ###
def infer_text_process(text, hps):

    lang = hps.data.languages
    add_blank = hps.data.add_blank
    
    # get_text function
    text_norm, tone = text_to_sequence(text, lang)
    if add_blank:
        text_norm = commons.intersperse(text_norm, 0)
        tone = commons.intersperse(tone, 0)
    
    text_length = torch.tensor(len(text_norm), dtype=torch.int64)
    text_length = torch.unsqueeze(torch.tensor(len(text_norm), dtype=torch.int64),dim=0)
    text_norm = torch.unsqueeze(torch.LongTensor(text_norm), dim=0)
    tone = torch.unsqueeze(torch.LongTensor(tone), dim=0)


    return text_norm, tone, text_length
###########